...
exports.GhostKeyTools = void 0;
exports.sessionKeystore = sessionKeystore;
...
class GhostKeyTools {
    ...
    token(symbolOrAddress) {
        ...
        const found = this.ctx.config.tokens.find((t) => t.symbol.toLowerCase() === needle || t.address.toLowerCase() === needle);
        if (found === undefined) {
            throw new Error(`unknown token "${symbolOrAddress}"; this session knows ${this.ctx.config.tokens.map((t) => t.symbol).join(", ")}`);
        }
        return found;
    }
    /**
    ...
    recipient(raw) {
        ...
        if (!/^0x[0-9a-fA-F]{40}$/.test(value)) {
            throw new Error(`"${raw}" is not an address. This needs a 0x-prefixed, 40-hex-digit address — names ` +
        ...
        try {
            ...
            throw new Error(`"${raw}" is the right shape but its checksum is wrong, which usually means one ` +
    ...
    requireLive() {
        if (this.live === null)
            throw new Error("no session is open; call open_session first");
        return this.live;
    }
    newRefId(kind) {
    ...
    async openSession(args) {
        ...
        const budgets = entries.map((t, i) => ({
            token: t.address,
            amount: (0, config_1.parseAmount)(args.budgets[i] ?? "0", t.decimals),
        }));
        for (const b of budgets) {
            if (b.amount === 0n)
                throw new Error("a zero budget would make the session useless");
        }
        const summary = entries.map((t, i) => `${args.budgets[i]} ${t.symbol}`).join(", ");
        const owner = await this.ctx.vault.unlock(`Open a session: ${summary} to ${args.allowlist.length} recipient(s), for ${args.ttlHours}h.` +
        ...
        return {
            ok: true,
            text: `Session open. ${summary} spendable to ${args.allowlist.length} address(es) for ${args.ttlHours}h` +
    ...
    async listAssets() {
        ...
        for (const t of this.ctx.config.tokens) {
            ...
            let onChain = "";
            try {
                const c = new ethers_1.Contract(t.address, ["function name() view returns (string)"], this.ctx.provider);
                const raw = (await c.name?.());
                const s = (0, sanitize_1.sanitiseChainText)(raw);
                if (s.text.toLowerCase() !== t.symbol.toLowerCase()) {
                    onChain = `  (${(0, sanitize_1.untrusted)("on-chain name", s.text)})`;
                }
            }
            catch {
    ...
    async balance(args) {
        ...
        if (live.tier !== "balance-visible" || live.session.tier !== "balance-visible") {
            return {
                ok: false,
                text: `This session cannot read your balance — it was opened without delegation.\n` +
    ...
    async canAfford(args) {
        const t = this.token(args.token);
        const wanted = (0, config_1.parseAmount)(args.amount, t.decimals);
        const live = this.requireLive();
    ...
    async sessionStatus() {
        ...
        return {
            ok: true,
            text: `Session ${live.session.sessionKeyAddress}\n` +
                `  owner       ${p.owner}\n` +
    ...
    async send(args) {
        ...
        if (spec.toLowerCase() === "sealed") {
            ...
            const answer = await this.ctx.console.ask("sealed", `Send ${t.symbol} to ${to}. Type the amount here; the model will not see it.`);
            if (!answer.approved || answer.value === undefined) {
                return { ok: false, text: "Cancelled at the console. Nothing was sent." };
            }
            expr = (0, sdk_1.exact)((0, config_1.parseAmount)(answer.value, t.decimals));
        }
            ...
            expr = (0, sdk_1.exact)((0, config_1.parseAmount)(spec, t.decimals));
        }
        ...
        return {
            ok: true,
            text: `Sent ${(0, config_1.formatAmount)(result.amount, t.decimals)} ${t.symbol} to ${to}.`,
            data: { status: "sent", amount: (0, config_1.formatAmount)(result.amount, t.decimals), tx: result.hash },
        };
    }
    // -------------------------------------------------------------------------
    ...
    async wrap(args) {
        ...
        const wrapper = new ethers_1.Contract(t.address, WRAPPER_ABI, this.ctx.provider);
        const underlying = new ethers_1.Contract(t.underlying, ERC20_ABI, this.ctx.provider);
        const decimals = Number((await underlying.decimals?.()) ?? t.decimals);
        const amount = (0, config_1.parseAmount)(args.amount, decimals);
        const owner = await this.ctx.vault.unlock(`Wrap ${args.amount} into ${t.symbol}. This moves a public balance, so it needs the vault.`);
        const approveTx = await underlying.connect(owner).approve?.(t.address, amount);
        await approveTx?.wait();
        const wrapTx = await wrapper.connect(owner).wrap?.(await owner.getAddress(), amount);
    ...
    async revokeAll() {
        if (this.live === null)
            return { ok: true, text: "There is no session to revoke." };
        const result = await this.live.session.close();
        const key = this.live.session.sessionKeyAddress;
        this.live = null;
        await this.pushStatus();
        const returned = result.reclaimed > 0n
            ? `\n${(0, ethers_1.formatEther)(result.reclaimed)} ETH of unused gas went back to your vault.`
        ...
        return {
            ok: true,
            text: `Session ${key} is closed. It can no longer move anything.` +
                returned +
    ...
    async revealRef(id, question, t) {
        ...
        const value = await (0, sdk_1.revealAmount)(r, { reason: question });
        return {
            ok: true,
            text: `${(0, config_1.formatAmount)(value, t.decimals)} ${t.symbol}`,
            data: { amount: (0, config_1.formatAmount)(value, t.decimals), token: t.symbol },
        };
    }
    /**
    ...
    async vaultPanel() {
        ...
        const ethBalance = address === "" ? "0" : (0, ethers_1.formatEther)(await this.ctx.provider.getBalance(address));
     ...
     */
    async mint(symbol, amount) {
        if (this.ctx.config.chainId !== 11155111) {
            throw new Error("minting test tokens is restricted to Sepolia");
        }
        const t = this.token(symbol);
        const value = (0, config_1.parseAmount)(amount, t.decimals);
        if (value === 0n)
            throw new Error("mint an amount greater than zero");
        const owner = await this.ctx.vault.unlock(`Mint ${amount} ${t.symbol} to your vault. Test tokens, Sepolia only.`);
        try {
            const token = new ethers_1.Contract(t.address, MINTABLE_ABI, owner);
            const fn = token.getFunction("mintPlain");
            const tx = (await fn(await owner.getAddress(), value));
            await tx.wait();
            return tx.hash;
        }
        finally {
    ...
    async vaultSummary() {
        ...
        return { address, balance: (0, ethers_1.formatEther)(await this.ctx.provider.getBalance(address)) };
    }
}
exports.GhostKeyTools = GhostKeyTools;
/** Turns an SDK error into something worth saying to a person. */
function explainFailure(e, symbol) {
    if (e instanceof sdk_1.ZeroAmountError) {
    ...
    if (e instanceof sdk_1.ProtocolUnavailableError) {
        return `The FHEVM protocol is refusing this right now: ${e.message}. Nothing to do with your budget.`;
    }
    if (e instanceof sdk_1.SessionNotLiveError) {
        return `The session is ${e.reason.replace(/-/g, " ")}. Open a new one.`;
    }
    return `The send failed: ${e.message}`;
}
/** Builds the keystore the SDK should use. Memory only for tests. */
function sessionKeystore(inMemory) {
    return inMemory
        ? (0, sdk_1.memoryKeystore)()
        : (0, sdk_1.osKeychainKeystore)({ dir: path.join(os.homedir(), ".ghostkey", "sessions") });
}
