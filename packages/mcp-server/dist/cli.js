...
async function init(args) {
    ...
    const provider = new ethers_1.JsonRpcProvider(rpcUrl, SEPOLIA);
    const vault = new vault_1.Vault({ provider, chainId: SEPOLIA });
    ...
    process.stdout.write([
        ...
        "  GhostKey is set up.",
        "",
        `  config           ${config_1.DEFAULT_CONFIG_PATH}`,
...
async function consoleOnly() {
    const handles = await (0, index_1.createServer)();
    process.stdout.write([
        "",
        "  GhostKey console",
        `  ${handles.console.url}`,
...
async function status() {
    ...
    const vault = new vault_1.Vault({ provider, chainId: cfg.chainId });
    const address = await vault.address();
    const balance = address === null ? "0" : (0, ethers_1.formatEther)(await provider.getBalance(address));
    process.stdout.write([
        "",
        `  chain      ${cfg.chainId}`,
