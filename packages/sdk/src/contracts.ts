...
import { ACL_ABI, ERC7984_ABI, GHOSTKEY_ABI } from "./abi";
...
export interface GhostKeyContract {
  ...
  protocolStatus(
    sessionKey: string,
  ): Promise<[aclPaused: boolean, keyDenied: boolean, moduleDenied: boolean]>;
  openSessionDigest(
    owner: string,
    sessionKey: string,
    expiry: number,
...
export function erc7984(address: string, runner: Signer | Provider): Erc7984Contract {
  return new Contract(address, ERC7984_ABI, runner) as unknown as Erc7984Contract;
}

export function acl(address: string, runner: Signer | Provider): AclContract {
  return new Contract(address, ACL_ABI, runner) as unknown as AclContract;
}
